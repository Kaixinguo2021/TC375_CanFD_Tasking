...
static void UDP_server_init(void)
{
    ...
    IP4_ADDR(&rmtipaddr, 192,168,0,10);
    /* 创建UDP协议控制块 */
    g_UDPPcb = udp_new();

    if (g_UDPPcb != NULL) 
    {
        #define  UDP_LOCAL_PORT    90
        /*  pcb：指定一个连接(pcb)
            ipaddr：绑定的本地IP地址。如果为IP_ADDR_ANY，则将连接绑定到所有的本地IP地址上
            port：绑定的本地端口号。
            注意：千万不要和其它的应用程序产生冲突  */
        err_t err = udp_bind(g_UDPPcb, IP_ADDR_ANY, UDP_LOCAL_PORT);
        if (err == ERR_OK)                                          
        {
            /* ipaddr:设置连接的远程主机IP地址 
               port:设置连接的远程主机端口号 */
            udp_connect(g_UDPPcb, &rmtipaddr, UDP_LOCAL_PORT);
            /* 注册接收回调函数 */
            udp_recv(g_UDPPcb, UDP_server_receive_callback, NULL);
        }
        else{
            /* 删除控制块 */
            udp_remove(g_UDPPcb);
